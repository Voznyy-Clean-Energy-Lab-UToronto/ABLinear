...
for i in targets:
    ...
    abs_val[filtset].to_csv("rs_{}.csv".format(i), index = 0)
...
for i in targets:
    filtset = [label] + [i] + atoms + orbs + [struct]
    abs_val_atomic[filtset].to_csv("rsa_{}.csv".format(i), index = 0)
...
for i in targets:
    filtset = [label] + [i] + atoms + orbs + [struct]
    abs_val_ai[filtset].to_csv("rsai_{}.csv".format(i), index = 0)
