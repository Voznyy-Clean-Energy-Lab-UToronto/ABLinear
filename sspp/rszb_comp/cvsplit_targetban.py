...
def splitter(ifilename, flag, drop):
    ...
    for i in range(len(fracs)):
        ...
        train.to_csv("{}_fold_{}_{}_train_{}.csv".format(split, drop[0], flag, i), header = 0, index = 0)
...
for i in droplist:
    splitter("zb_gap.csv", "zb_only", [i])
    splitter("rs_gap.csv", "zb_both", [i])
    splitter("rs_gap.csv", "both_both", [i, "rs_"+i ])